...
function [art, horiz, vert, blink, disc,...
        soglia_DV, diff_var, soglia_K, med2_K, meanK, soglia_SED, med2_SED, SED, soglia_SAD, med2_SAD, SAD, ...
        soglia_GDSF, med2_GDSF, GDSF, soglia_V, med2_V, nuovaV, soglia_D, maxdin, maxvar, activations]=ADJUST (EEG,out)
...
numic = size(EEG.icawinv,1);

%number of time points=size(EEG.data,2);

Edata = EEG.data(1:size(EEG.icawinv,1),:,:);
...
%         EEG.icaact = reshape(EEG.icaact,size(EEG.icawinv,1),size(EEG.data,2), num_epoch);
         EEG.icaact = reshape(EEG.icaweights*EEG.icasphere*reshape(Edata,[size(Edata,1)...
 size(Edata,2)*size(Edata,3)]),[size(Edata,1) size(Edata,2) size(Edata,3)]);
    else EEG.icaact = EEG.icaweights*EEG.icasphere*EEG.data(1:numic,:);
    end
end
...
fprintf(fid,'Automatic EEG artifacts Detector with Joint Use of Spatial and Temporal features\n\n');

fprintf(fid,'Andrea Mognon and Marco Buiatti (2009-2014)\n\n');

fprintf(fid,['Analyzed dataset: ' EEG.filename '\n']);

fprintf(fid,['Analysis date: ' date '\n']);

fprintf(fid,'Analysis carried out on the %d Independent Components\n\n',size(EEG.icawinv,2));
...
fprintf(fid,'> HEM - Horizontal movements\n\n');

fprintf(fid,'Classification based on features:\n');

fprintf(fid,'SED - Spatial eye difference (threshold=%f)\n',soglia_SED);

fprintf(fid,'MEV - Maximum epoch variance (threshold=%f)\n\n',soglia_V);

fprintf(fid,'ICs with Horizontal eye movements:\n');
...
fprintf(fid,'>> VEM - Vertical movements\n\n');

fprintf(fid,'Classification based on features:\n');

fprintf(fid,'SAD - Spatial average difference (threshold=%f)\n',soglia_SAD);

fprintf(fid,'MEV - Maximum epoch variance (threshold=%f)\n\n',soglia_V);

fprintf(fid,'ICs with Vertical eye movements:\n');
...
fprintf(fid,'>>> EB - Blinks\n\n');

fprintf(fid,'Classification based on features:\n');

fprintf(fid,'SAD (threshold=%f)\n',soglia_SAD);

fprintf(fid,'TK - Temporal kurtosis (threshold=%f)\n\n',soglia_K);

fprintf(fid,'ICs with Blinks:\n');
...
fprintf(fid,'>>>> GD - Discontinuities\n');

fprintf(fid,'Classification based on features:\n');

fprintf(fid,'GDSF - Generic Discontinuities Spatial Feature (threshold=%f)\n',soglia_GDSF);

fprintf(fid,'MEV - Maximum epoch variance (threshold=%f)\n\n',soglia_V);

fprintf(fid,'ICs with Generic Discontinuities:\n');
...
if isempty(disc) %no discontinuities
    ...
    fprintf(fid,[num2str(disc) '\n']);
    fprintf(fid,'\n');    
end

aic=unique([blink disc horiz vert]);

fprintf(fid,'Artifacted ICs (total):\n');
    fprintf(fid,[num2str(aic) '\n']);
    fprintf(fid,'\n');    
